...
abstract class controller {
    ...
    public function __construct() {
       ...
       // print_r($menu);exit;
        ...
        @$padre=$datos_modulos[0]['ID_PADRE'];
        $this->_vista = new view(new request, $menu,$padre,$alerta);
    }
...
//            header('location:' . BASE_URL);
    ...
    public function acceso() {
        ...
        $permisos->id_perfil_usuario= session::get('idperfil');
        $permisos->url= $url;
        $permiso = $permisos->valida_acceso();
//        print_r($permiso);exit;
        if ($permiso[0]['ID_PERFIL_USUARIO']!=session::get('idperfil') || $permiso[0]['ESTADO'] ==0) {
            return false;
        } else {
            return true;
        }
